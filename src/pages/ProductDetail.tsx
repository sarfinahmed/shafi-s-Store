...
export function ProductDetail() {
  ...
  if (loading) return <div className="text-center py-20 text-zinc-600 dark:text-zinc-400 font-medium">Loading premium product...</div>;
  ...
  if (product.isActive === false && !user?.isAdmin) {
    return <div className="text-center py-20 text-zinc-600 dark:text-zinc-400 font-bold">This product is currently unavailable.</div>;
  }
  ...
  if (isLocked) {
    return (
      <div className="max-w-2xl mx-auto py-20 px-4 text-center">
        <Link to="/" className="inline-flex items-center text-xs font-medium text-zinc-600 dark:text-zinc-400 hover:text-black dark:text-white mb-8 transition-colors">
          <ArrowLeft className="w-3 h-3 mr-2" /> Back to Products
        </Link>
        <div className="bg-zinc-50 dark:bg-[#0a0a0a] border border-zinc-200 dark:border-zinc-900 rounded-3xl p-8 md:p-12 shadow-2xl flex flex-col items-center justify-center">
          ...
          <p className="text-sm text-zinc-600 dark:text-zinc-400 font-medium max-w-sm mb-6">
            আমাদের ওয়েবসাইট থেকে যারা ৫০০০ টাকার জিনিস কিনবে শুধু তারাই এইখান থেকে কিনতে পারবে।
          </p>
          <div className="bg-zinc-100 dark:bg-[#111] border border-zinc-300 dark:border-zinc-800 rounded-xl px-4 py-3">
            <p className="text-xs font-bold text-zinc-700 dark:text-zinc-300 uppercase tracking-widest">
  ...
  return (
    <div className="max-w-2xl md:max-w-4xl mx-auto py-2 md:py-6 px-4 md:px-0">
      <Link to="/" className="inline-flex items-center text-xs font-medium text-zinc-600 dark:text-zinc-400 hover:text-black dark:text-white mb-4 transition-colors">
        <ArrowLeft className="w-3 h-3 mr-2" /> Back to Products
      </Link>
      
      {/* Top Banner */}
      <div className="w-full relative h-48 md:h-64 rounded-t-2xl overflow-hidden mb-6 border border-zinc-200 dark:border-zinc-900 bg-zinc-100 dark:bg-[#111]">
        {product.imageUrl ? (
          <img 
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#050505] via-[#050505]/60 to-transparent flex flex-col justify-end p-6">
          <h1 className="text-2xl md:text-3xl font-black tracking-tighter text-black dark:text-white mb-1 shadow-sm">{product.title}</h1>
          ...
          {product.description && (
             <p className="text-zinc-700 dark:text-zinc-300 text-sm max-w-2xl hidden md:block">{product.description}</p>
          )}
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        {product.options && product.options.length > 0 ? (
          <div className="bg-zinc-50 dark:bg-[#0a0a0a] rounded-2xl border border-zinc-200 dark:border-zinc-900 overflow-hidden">
            <div className="px-5 py-4 border-b border-zinc-200 dark:border-zinc-900 flex items-center bg-zinc-100 dark:bg-[#0d0d0d]">
              ...
              <h2 className="text-sm font-bold text-black dark:text-white uppercase tracking-widest">Select Recharge</h2>
            </div>
                  ...
                  <button
                    ...
                    className={`flex flex-row items-center justify-between p-4 rounded-xl border transition-all ${
                      selectedOption === opt 
                        ...
                        : 'bg-zinc-100 dark:bg-[#111] border-zinc-300 dark:border-zinc-800 hover:border-zinc-500 hover:bg-zinc-200 dark:bg-zinc-900'
                    }`}
                  >
                    <span className={`text-xs md:text-sm font-bold uppercase tracking-wide ${selectedOption === opt ? 'text-black dark:text-white' : 'text-zinc-700 dark:text-zinc-300'}`}>{opt.name}</span>
                    <div className="flex flex-col items-end">
                      ...
                      {(() => {
                        ...
                        return (
                          <span className={`text-[9px] font-bold uppercase tracking-tighter mt-0.5 ${stockDisplay === 'Out of Stock' || stockDisplay === 'Sold Out' ? 'text-red-500' : (stockDisplay === 'Unlimited' ? 'text-blue-500' : 'text-zinc-600 dark:text-zinc-400')}`}>
                            {stockDisplay}
                          </span>
                        );
                      })()}
           ...
           <div className="bg-zinc-50 dark:bg-[#0a0a0a] rounded-2xl border border-zinc-200 dark:border-zinc-900 p-5 flex justify-between items-center">
              <div className="flex flex-col">
                <h2 className="text-sm font-bold text-black dark:text-white uppercase tracking-widest">Price</h2>
                {(() => {
                  let stockDisplay = null;
        ...
        {product.requiredUserInputLabel && (
          <div className="bg-zinc-50 dark:bg-[#0a0a0a] rounded-2xl border border-zinc-200 dark:border-zinc-900 overflow-hidden">
            <div className="px-5 py-4 border-b border-zinc-200 dark:border-zinc-900 flex items-center bg-zinc-100 dark:bg-[#0d0d0d]">
              ...
              <h2 className="text-sm font-bold text-black dark:text-white uppercase tracking-widest">Account Info</h2>
            </div>
            <div className="p-5 space-y-4">
              <div>
                <label className="block text-xs font-bold text-zinc-600 dark:text-zinc-400 mb-2">{product.requiredUserInputLabel}</label>
                <input 
                  type="text" 
                  value={userInput}
                  onChange={(e) => setUserInput(e.target.value)}
                  className="w-full bg-zinc-100 dark:bg-[#111] border border-zinc-300 dark:border-zinc-800 text-black dark:text-white text-sm rounded-lg px-4 py-3 focus:outline-none focus:border-zinc-500 transition-colors placeholder:text-zinc-700"
                  placeholder={`Enter ${product.requiredUserInputLabel}`}
                />
              </div>
              {product.description && (
                <div className="text-xs text-zinc-600 dark:text-zinc-400 whitespace-pre-wrap md:hidden">
                   {product.description}
                </div>
              )}
            </div>
              ...
              <div className="flex flex-col items-center justify-center bg-green-950/30 border border-green-900/50 text-green-400 py-4 px-4 rounded-xl font-bold text-sm mb-6">
                <div className="flex flex-col items-center text-center">
                  ...
                  {product.isManualFulfillment && (
                      <div className="text-zinc-600 dark:text-zinc-400 font-medium text-[10px] uppercase tracking-widest mt-2 bg-white/20 dark:bg-black/20 px-3 py-1.5 rounded-lg border border-white/5">
                ...
                {orderLink && (
                    <div className="mt-3 break-all bg-green-950/50 px-3 py-2 rounded-lg text-center text-xs">
                      <span className="text-zinc-600 dark:text-zinc-400 font-medium block mb-1">Your Link:</span>
                      <a href={orderLink} target="_blank" rel="noreferrer" className="text-green-300 hover:text-black dark:text-white underline underline-offset-4 pointer-events-auto cursor-pointer block">{orderLink}</a>
                    </div>
                )}
              </div>
            )}
                  ...
                  {!user ? (
                    <div className="flex gap-2 w-full">
                      ...
                      <Link to="/login" className="flex-1">
                        <Button className="w-full text-[10px] md:text-sm py-6 bg-zinc-200 dark:bg-zinc-900 hover:bg-zinc-300 dark:hover:bg-zinc-800 text-black dark:text-white font-black uppercase tracking-widest border border-zinc-300 dark:border-zinc-800">
                          Login
                        </Button>
                      </Link>
                    </div>
                    ...
                    <Button onClick={handlePurchase} className="flex-1 text-sm py-6 bg-green-600 hover:bg-green-500 text-black dark:text-white font-black uppercase tracking-widest shadow-[0_0_20px_rgba(34,197,94,0.2)]">
