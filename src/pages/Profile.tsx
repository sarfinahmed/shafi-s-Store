        ...
        <div className="space-y-4 md:space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            ...
            >
              ...
              <p className="text-[10px] text-zinc-500 font-bold mt-2 hover:text-white transition-colors">{activeView === 'deposits' ? 'Hide History' : 'Tap to filter History'}</p>
            </div>
            ...
            >
              ...
              <p className="text-[10px] text-zinc-500 font-bold mt-2 hover:text-white transition-colors">{activeView === 'spent' ? 'Hide History' : 'Tap to filter History'}</p>
            </div>
            ...
            >
              ...
              <p className="text-[10px] text-zinc-500 font-bold mt-2 hover:text-white transition-colors">{activeView === 'orders' ? 'Hide Orders' : 'Tap to view Orders'}</p>
            </div>
            ...
            >
              ...
              <p className="text-[10px] text-zinc-500 font-bold mt-2 hover:text-white transition-colors">{activeView === 'codes' ? 'Hide Codes' : 'Tap to view Codes'}</p>
            </div>
          </div>
          ...
          {showDeposit && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="bg-[#0a0a0a] p-5 md:p-6 rounded-2xl md:rounded-3xl border border-zinc-900 space-y-4 md:space-y-6">
              <div className="text-center mb-6">
                ...
                <p className="text-zinc-500 text-xs md:text-sm mt-2 font-medium">Send money to any of the numbers below and enter the transaction details.</p>
              </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                  {settings.paymentMethods.map(pm => (
                    <div key={pm.id} className="bg-[#111] border border-zinc-800 rounded-xl p-4 flex flex-col items-center text-center relative group">
                      ...
                      <p className="text-zinc-500 text-xs font-mono mb-2">{pm.details}</p>
                      <button 
                        onClick={() => {
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 bg-zinc-900 border border-zinc-800 text-zinc-500 hover:text-white hover:bg-zinc-800 rounded-lg transition-colors text-[10px] font-bold uppercase tracking-wider"
                        title="Copy Number"
                      >
          ...
          <div className="bg-red-950/20 border border-red-900/50 rounded-2xl md:rounded-3xl p-5 md:p-6">
            ...
            {showDelete ? (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-4">
                ...
                <p className="text-xs text-zinc-500">This action cannot be undone. Your balance, purchased products, and profile data will be permanently deleted.</p>
                <div className="flex flex-col sm:flex-row gap-2 pt-2">
                  <Button variant="danger" onClick={async () => {
                    await deleteAccount();
                  }}>Yes, delete my account</Button>
                  <Button variant="ghost" className="text-zinc-500 hover:text-white" onClick={() => setShowDelete(false)}>Cancel</Button>
                </div>
              </motion.div>
            ) : (
